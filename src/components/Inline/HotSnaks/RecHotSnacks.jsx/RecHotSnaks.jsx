...
import s from './RecHotSnaks.module.css';
...
const RecHotSnacks = ({isAddButtonDisabled}) => {
  ...
  useEffect(() => {
    const fetchRecomendations = async () => {
      try {
        ...
        const snapshot = await get(child(dbRef, "soups")); // Измените путь на "recomendations"
        if (snapshot.exists()) {
  ...
  const handleCardClick = (card) => {
    navigate('/hotsnacksIn', { state: { dish: card, fromRecomendations: true, isAddButtonDisabled } });
  };
